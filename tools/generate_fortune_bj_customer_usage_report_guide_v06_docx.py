...
OUT_DIR = ROOT / "docs"
OUT_PATH = OUT_DIR / "Fortune_BJ_产能优化工具客户使用与报告阅读说明_CN_V06.docx"
...
def build_body() -> str:
    ...
    parts.append(paragraph("版本：CN V06 | 对应工具版本：v0.1.3 | 更新日期：2026-07-27", style="Subtitle"))
    parts.append(callout("这份说明面向工具使用人，只说明输入文件怎么维护、工具怎么运行、报告怎么看。工具输出用于产能分析、资源分流和改善建议，不作为车间逐小时执行排程单。"))
    ...
    parts.append(section("2. 使用前准备", [
        ...
        table(["文件", "客户维护什么", "使用例子"], [
            ...
            ["可选工序_排产输入模板.xlsx", "维护物料和活动的可选工作中心，或维护外包路径；可选单位工时可填写真实工时，也可留空让工具参考生产订单工序补齐。", "物料A活动20可选北京小五轴；外包行把可选资源组分类填为外包，单位工时不用填。"],
    ...
    parts.append(subsection("2.2 可选工序怎么维护", [
        paragraph("可选工序表用于告诉工具：某个物料的某道活动，除了原工作中心以外，还可以选择哪些厂内资源或外包资源。客户维护时重点确认物料、活动、可选工作中心、可选资源组分类和可选单位工时。"),
        table(["维护场景", "怎么填写", "使用例子"], [
            ["可选厂内工作中心，已知真实工时", "填写可选工作中心、可选资源组分类和可选单位工时。", "物料M0106-006081活动20可选PRO-800-FIU，真实工时13.961小时/件，就填写13.961。"],
            ["可选厂内工作中心，希望沿用原工时", "可选单位工时保持空白。工具会在报告中标记该工时来源，方便复核。", "物料A活动20原工时为8小时/件，可选工序工时留空，报告中显示单位工时来源为从生产订单工序继承。"],
            ["可选工时暂时只是占位数", "不要填1或0作为占位。需要自动补齐时请留空；需要指定真实工时时请直接填真实数值。", "如果填1，报告会按1小时/件计算；如果这不是实际工时，应清空或改成真实工时。"],
            ["外包路径", "可选资源组分类填写外包；可选工作中心和单位工时可以不填。", "物料A活动20可外包，资源组分类填外包，报告会把外包行作为不占厂内产能的路径。"],
        ], widths=[2300, 4200, 2860]),
        callout("如果可选单位工时留空，但生产订单工序里找不到相同物料和活动的工时，工具会生成校验报告并停止运行。客户按校验报告补齐可选单位工时或源工序工时后，再重新运行。"),
    ]))
    ...
    parts.append(section("3. 怎么运行工具", [
        table(["步骤", "操作", "例子"], [
            ...
            ["6", "选择优化开始周期。", "从2026-07-20开始看未完成订单，就把日期选到2026-07-20；ModeA和ModeB都会使用这个开始周期。"],
            ["7", "点击运行产能分析。", "运行完成后，弹窗可直接打开最新报告。"],
        ], widths=[800, 3600, 4960]),
    ]))

    parts.append(section("4. ModeA怎么用", [
        paragraph("ModeA用于先看原始订单压力。它适合做第一轮产能盘点，帮助客户快速找到高负荷工作组、工作中心和订单。ModeA也会使用界面上选择的优化开始周期，便于和ModeB在同一时间范围内对比。"),
        table(["报告页", "客户怎么用", "例子"], [
    ...
    parts.append(section("5. ModeB怎么用", [
        paragraph("ModeB用于看100%产能优化建议。它会结合可选工序和外包路径，给出周期级的产能分流结果。客户重点看优化后负荷是否下降、哪些工序被分流、哪些周期仍然需要额外处理。"),
        table(["报告页", "客户怎么用", "例子"], [
    ...
    parts.append(section("7. 订单工序分配明细怎么读", [
        ...
        table(["字段", "客户怎么理解", "例子"], [
            ...
            ["优化后单位工时来源", "看优化后单位工时来自哪里，便于复核可选工序维护。", "显示可选工序表填写，表示客户在可选工序表手动维护；显示从生产订单工序继承，表示可选单位工时原来为空。"],
